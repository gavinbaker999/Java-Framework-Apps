		...
		public void editor() {
			...
			if (nl.getLength() != 0) {
				...
				if (isActor()) {
					...
					String[] data = getDataAsDialog("Add UML Use Case Actor",titles,defaults);
					if (data != null) {
					...
					String[] data = getDataAsDialog("Add UML Use Case",titles,defaults);
					if (data != null) {
	...
	public class UMLSequenceDrawingItem extends drawingItem {
		...
		public void editor() {
			...
			if (nl.getLength() != 0) {
				...
				String[] data = getDataAsDialog("Add UML Sequence",titles,defaults);
				if (data != null) {
					org.w3c.dom.Element eNew = umlDiagram.getSequenceDocument().createElement("sequence");
					eNew.setAttribute("name",data[0]);
					eNew.setAttribute("description",data[1]);
	...
	public class UMLStateDrawingItem extends drawingItem {
		...
		public void editor() {
			...
			if (nl.getLength() != 0) {
				...
				String[] data = getDataAsDialog("Add UML State",titles,defaults);
				if (data != null) {
					org.w3c.dom.Element eNew = umlDiagram.getStateDocument().createElement("state");
					eNew.setAttribute("name",data[0]);
					eNew.setAttribute("description",data[1]);
	...
	public class UMLDrawingCanvas extends drawingCanvas {
		   ...
		   public boolean doPaste(drawingItem d) {
			...
			public boolean printPageSetupOverride(Graphics2D g2d,PageFormat format,Dimension size) {
				String tmp = getPropValue("Scaling Factor","Scaling Factor","1");
	...
	public class umlDiagramStruct implements propBoxDialogListener {
		...
		public void diagramProperties() {
			if (umlDiagram.getDiagramType() == UMLDiagramType.STATE) {	
				...
				String[] data = getDataAsDialog("State Diagram Title",titles,defaults);
				if (data != null) {
			...
			if (umlDiagram.getDiagramType() == UMLDiagramType.USECASE) {
				...
				String[] data = getDataAsDialog("Use Case Diagram Title",titles,defaults);
				if (data != null) {
			...
			if (umlDiagram.getDiagramType() == UMLDiagramType.SEQUENCE) {	
				...
				String[] data = getDataAsDialog("Sequence Diagram Title",titles,defaults);
				if (data != null) {
...
//				}
		...
		public void splitIntoSheets(Vector drawingItems) {
			Rectangle rcSheet = new Rectangle(0,0,dcMaxX,dcMaxY);
			int numXSheets = (rcUMLDiagram.width / dcMaxX) + 1;
			int numYSheets = (rcUMLDiagram.height / dcMaxY) + 1;
			...
			for (int i=0;i<drawingItems.size();i++) {
				...
				int xNewCord = rc.x - (sx * dcMaxX);
				int yNewCord = rc.y - (sy * dcMaxY);
		...
		public boolean processCordData(Vector drawingItems) {
			...
			for (int i=0;i<drawingItems.size();i++) {
				...
				if (xCord + rc.width >= (((rc.x / dcMaxX) + 1) * dcMaxX)) {
	...
	public class mainCard extends Panel  implements drawingCanvasUtils,ActionListener,statusCanvasDialogListener,ItemListener {
		...
		mainCard() {
			...
			sDC = new scrollableDrawingCanvas(supportFunctions.getTmpFilename("untitled"),dcMaxX,dcMaxY,20,20,false);
